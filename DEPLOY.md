...
3. **Зареєструйте webhook** (один раз, у PA Bash console):
   ...
   ```
4. **Напишіть боту `/start`** (він створить ваш запис).
5. В адмінці сайту → **✈️ Telegram: користувачі та права** → знайдіть себе → увімкніть
   **«Адміністратор»** і збережіть. Бот надішле вам підтвердження — тепер ви отримуєте
   сповіщення й керуєте зверненнями кнопками. Готово!

> 👨‍👩‍👧 Батьки можуть користуватись ботом (запитання, відгуки) **без** жодних прав — одразу після `/start`.
> 🛡 Адмін-функції доступні лише тим, кому ви увімкнули «Адміністратор».
